...
export function LetterPreviewScreen() {
  ...
  return <FocusShell title="편지 미리보기" fallback="/write-letter" className="letter-preview-screen" action={<div className="flow-fixed-action flow-fixed-action--split"><button type="button" className="flow-secondary-button" onClick={() => { updateLetterDraft(userId, { stage: "writing" }); navigateTo("/write-letter"); }}>수정하기</button><button type="button" className="flow-primary-button" onClick={submit}>편지 보내기</button></div>}><section className="flow-review"><h1><strong>편지</strong>를 보내기 전에<br />살펴봐주세요</h1><p className="reply-review-intro">서두르지 않아도 괜찮아요.</p><img className="reply-review-header-illustration" src="/assets/reply-review-open-letter-glasses.png" alt="" aria-hidden="true" /><article className="flow-letter-paper"><span className="my-letter-waiting-quote my-letter-waiting-quote--open" aria-hidden="true">“</span><blockquote>{draft.content}</blockquote><span className="my-letter-waiting-quote my-letter-waiting-quote--close" aria-hidden="true">”</span><span className="letter-preview-signature">─ {draft.anonymousName.trim() || getCurrentAnonymousName()}</span><small className="reply-review-edited-at">{formatDateTime(draft.updatedAt)}에 마지막으로 다듬었어요.</small></article><section className="reply-review-guidance" aria-label="보내기 전 점검"><h2>보내기 전, 잠시 살펴봐주세요</h2><ul className="review-list"><li>보내고 나면 내용을 고칠 수 없어요.</li><li>부적절하거나 불법적인 내용은 없나요?</li><li>개인정보나 연락처를 적지 않았나요?</li><li>학교·회사처럼 나를 짐작하게 하는 내용은 없나요?</li></ul></section><p className="flow-notice" role="status">{notice}</p>{notice && <button className="flow-text-button" type="button" onClick={() => navigateTo("/write-letter")}>편지로 돌아가기</button>}</section></FocusShell>;
}
...
export function ReplyReviewScreen({ letterId }: { letterId?: string }) {
  ...
  return <FocusShell title="답장 미리보기" fallback={`/write-reply/${letter.id}`} className="reply-review-screen" action={<div className="flow-fixed-action flow-fixed-action--split reply-review-fixed-action"><button type="button" className="flow-secondary-button" onClick={() => { updateReplyDraft(letter.id, currentUserId, { stage: "writing", letterStatusAtSave: letter.status }); navigateTo(`/write-reply/${encodeURIComponent(letter.id)}`); }}>수정하기</button><button type="button" className="flow-primary-button" onClick={submit} disabled={submitting}>답장 보내기</button></div>}><section className="flow-review"><h1><strong>답장</strong>을 보내기 전에<br />살펴봐주세요</h1><p className="reply-review-intro">서두르지 않아도 괜찮아요.</p><img className="reply-review-header-illustration" src="/assets/reply-review-open-letter-glasses.png" alt="" aria-hidden="true" /><article className="flow-letter-paper reply-review-paper"><p className="reply-compose-recipient">{letter.anonymousName}님에게</p><blockquote>{draft.content}</blockquote><small className="reply-review-edited-at">{formatDateWithYear(draft.updatedAt)}에 마지막으로 다듬었어요.</small></article><section className="reply-review-guidance" aria-label="보내기 전 점검"><h2>보내기 전, 잠시 살펴봐주세요</h2><ul className="review-list"><li>보내고 나면 내용을 고칠 수 없어요.</li><li>부적절하거나 불법적인 내용은 없나요?</li><li>개인정보나 연락처를 적지 않았나요?</li><li>상대를 판단하거나 비난하지 않았나요?</li></ul></section><p className="flow-notice" role="status">{notice}</p></section></FocusShell>;
}
